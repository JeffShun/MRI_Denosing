...
class DenosingPredictor:
    ...
    def _forward(self, img):
        ori_shape = img.shape
        img_t = torch.from_numpy(img)[None,None].to(self.device).float()
        img_t = self._resize_torch(img_t, self.test_cfg.patch_size)
        # pytorch预测
        with torch.no_grad():
            output = self.net(img_t)
            output = self._resize_torch(output, ori_shape)
            output = output.squeeze().cpu().detach().numpy()      
        return output

    def _resize_torch(self, data, size, mode="bilinear"):
        return torch.nn.functional.interpolate(data, size=size, mode=mode)    