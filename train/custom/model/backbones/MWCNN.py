...
def iwt_init(x):
    ...
    h = torch.zeros([out_batch, out_channel, out_height, out_width]).float().to(x.dtype).to(x.device)
...
class MWCNN(nn.Module):
    def __init__(self, in_channels, out_channels, n_feats):
        ...
        m_head = [BBlock(default_conv, in_channels, n_feats, kernel_size, act=act)]
        d_l0 = []
